...
from reddhog.config import BROWSER_FIRST_LOAD_TIMEOUT_MS
...
def test_start_runs_browser_warmup(monkeypatch):
    """start() and close() run without error; no page is opened during start (warmup is separate)."""
    page = DummyPage()
    ...
    client._profile_dir = profile_dir
    try:
        asyncio.run(client.start())
        asyncio.run(client.close())
    finally:
        pass
    # No page is created during start(), so no goto
